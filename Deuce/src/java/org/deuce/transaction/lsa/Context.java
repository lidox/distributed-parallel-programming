...
final public class Context implements org.deuce.transaction.Context {
	...
	final private ReadSet readSet = new ReadSet(65536);//1024);
	final private WriteSet writeSet = new WriteSet(65536);//32);
	...
	/***************  COUNTER (MS) *************/
	public static int counter_BOT=0;
	public static int counter_EOT=0;
	public static int counter_Abort=0;
	private static Object lock = new Object();

	@Override
	public void init(int blockId, String metainf) {
		/***************  COUNTER (MS) *************/
		synchronized(lock) { counter_BOT++; }
		readSet.clear();
		writeSet.clear();
		...
		try{
			if (!writeSet.isEmpty()) {
				...
				if (newClock != startTime + 1 && !readSet.validate(id)) {
					/***************  COUNTER (MS) *************/
					synchronized(lock) { counter_Abort++; }
					//System.out.println("writeSet.rollback 111");
					writeSet.rollback(); // Release locks
					//System.out.println("writeSet.rollback 222");
					return false;
				}
	...
	public void rollback() {
		/***************  COUNTER (MS) *************/
		//System.out.println("    *** writeSet.rollback");
		synchronized(lock) { counter_Abort++; }
		// Release locks
		writeSet.rollback();
		irrevocableAccessLock.readLock().unlock();
	...
	private boolean onReadAccess(Object obj, long field, Type type, Object value) {
		if (readLock < 0) {
	...
	public Object onReadAccess(Object obj, Object value, long field) {
		return (onReadAccess(obj, field, Type.OBJECT, value) ? readValue : value);
	}

	@Override
	public boolean onReadAccess(Object obj, boolean value, long field) {
		return (onReadAccess(obj, field, Type.BOOLEAN, null) ? (Boolean) readValue : value);
	}

	@Override
	public byte onReadAccess(Object obj, byte value, long field) {
		return (onReadAccess(obj, field, Type.BYTE, null) ? ((Number) readValue).byteValue() : value);
	}

	@Override
	public char onReadAccess(Object obj, char value, long field) {
		return (onReadAccess(obj, field, Type.CHAR, null) ? (Character) readValue : value);
	}

	@Override
	public short onReadAccess(Object obj, short value, long field) {
		return (onReadAccess(obj, field, Type.SHORT, null) ? ((Number) readValue).shortValue() : value);
	}

	@Override
	public int onReadAccess(Object obj, int value, long field) {
		return (onReadAccess(obj, field, Type.INT, null) ? ((Number) readValue).intValue() : value);
	}

	@Override
	public long onReadAccess(Object obj, long value, long field) {
		return (onReadAccess(obj, field, Type.LONG, null) ? ((Number) readValue).longValue() : value);
	}

	@Override
	public float onReadAccess(Object obj, float value, long field) {
		return (onReadAccess(obj, field, Type.FLOAT, null) ? ((Number) readValue).floatValue() : value);
	}

	@Override
	public double onReadAccess(Object obj, double value, long field) {
		return (onReadAccess(obj, field, Type.DOUBLE, null) ? ((Number) readValue).doubleValue() : value);
	}
